...
class TextExtractor(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs):
        ...
        if tag == 'script':
            ...
            self._in_title = True
        elif tag in {'p', 'div', 'section', 'article', 'li', 'tr', 'br', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'pre'}:
            self.parts.append('\n')
...
def build_output_path(root: Path, url: str) -> Path:
    ...
    return root / 'manpages' / parsed.netloc / safe_path.with_suffix('.md')
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Fetch and cache an Ubuntu manpage into .Ubuntu-Encyclopedia.')
    parser.add_argument('--url', required=True, help='manpages.ubuntu.com URL to cache')
